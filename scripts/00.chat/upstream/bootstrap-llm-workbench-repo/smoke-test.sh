...
node -e "const p=require(process.argv[1]); if (p.name !== 'llm-wb') process.exit(1); if (p.version !== '0.1.0-beta.0') process.exit(1); if (!p.scripts.chat) process.exit(1); if (!p.bin || Object.keys(p.bin).length !== 1 || p.bin['llm-wb'] !== 'bin/llm-workbench.js') process.exit(1)" "$EMPTY_REPO/package.json" || fail "created package.json did not match template"
